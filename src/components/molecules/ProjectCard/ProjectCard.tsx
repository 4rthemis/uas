...
export const ProjectCard: React.FC<ProjectCardProps> = ({ project, className }) => {
  return (
    <Card className={`p-4 sm:p-6 md:p-8 ${project.bgColor} flex flex-col lg:flex-row w-full items-center justify-center gap-4 sm:gap-5 md:gap-6 rounded-2xl sm:rounded-3xl border-none shadow-none hover:shadow-lg transition-shadow duration-300 ${className}`}>
      <CardContent className="flex flex-col items-start gap-3 sm:gap-4 relative flex-1 p-0 w-full lg:w-auto">
        <div className="flex flex-col items-start gap-3 sm:gap-4 relative self-stretch w-full">
          ...
          <Text 
            variant="h4" 
            className={`${project.textColor} self-stretch`}
          >
            {project.title}
          </Text>
        </div>
